...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def init_ui(self):
        ...
        # Macros table.
        self.tableWidget_Macros.itemSelectionChanged.connect(
            self.tableWidget_Macros_OnSelect
        )
        ...
        # Page "Equipment"
        self.checkBox_Equipment_ManualMode.clicked.connect(
            self.Equipment_ManualMode_OnChange
        )
        ...
        self.button_EquipmentReloadInventory.clicked.connect(
            self.Equipment_ReloadInventory
        )
        self.comboBox_EquipmentInventoryCurrentType.activated.connect(
            self.EquipmentInventoryType_OnChange
        )
        self.tableWidget_Equipment.itemChanged.connect(
            self.Equipment_ManualMode_Table_OnChange
        )
        self.tableWidget_Equipment.doubleClicked.connect(
            self.Equipment_ManualMode_Table_DoubleClicked
        )
        self.comboBox_Equip_InstantAction.activated.connect(
            self.Equip_InstantAction_OnChange
        )
        self.comboBox_Equip_TwoHand.activated.connect(self.Equip_TwoHand_OnChange)
        self.checkBox_Equipment_NotEnoughStats.clicked.connect(
            self.Equipment_NotEnoughStats_OnChange
        )

        self.picture_equip_weaponright_1.mousePressEvent = (
            self.Equipment_MouseClicked_WeaponRight_1
        )
        self.picture_equip_weaponright_2.mousePressEvent = (
            self.Equipment_MouseClicked_WeaponRight_2
        )
        self.picture_equip_weaponright_3.mousePressEvent = (
            self.Equipment_MouseClicked_WeaponRight_3
        )
        self.picture_equip_weaponleft_1.mousePressEvent = (
            self.Equipment_MouseClicked_WeaponLeft_1
        )
        self.picture_equip_weaponleft_2.mousePressEvent = (
            self.Equipment_MouseClicked_WeaponLeft_2
        )
        self.picture_equip_weaponleft_3.mousePressEvent = (
            self.Equipment_MouseClicked_WeaponLeft_3
        )
        self.picture_equip_armor_head.mousePressEvent = (
            self.Equipment_MouseClicked_Armor_Head
        )
        self.picture_equip_armor_chest.mousePressEvent = (
            self.Equipment_MouseClicked_Armor_chest
        )
        self.picture_equip_armor_arms.mousePressEvent = (
            self.Equipment_MouseClicked_Armor_Arms
        )
        self.picture_equip_armor_legs.mousePressEvent = (
            self.Equipment_MouseClicked_Armor_Legs
        )
        self.picture_equip_talisman_1.mousePressEvent = (
            self.Equipment_MouseClicked_Talisman_1
        )
        self.picture_equip_talisman_2.mousePressEvent = (
            self.Equipment_MouseClicked_Talisman_2
        )
        self.picture_equip_talisman_3.mousePressEvent = (
            self.Equipment_MouseClicked_Talisman_3
        )
        self.picture_equip_talisman_4.mousePressEvent = (
            self.Equipment_MouseClicked_Talisman_4
        )
        ...
        # Page "Magic"
        self.tableWidget_AvaiableMagic.itemSelectionChanged.connect(
            self.AvailableMagic_OnChange
        )
        self.tableWidget_AvaiableMagic.setEditTriggers(QTableWidget.NoEditTriggers)
        self.checkBox_MagicInstantUseLeftHand.clicked.connect(
            self.MagicInstantUseLeftHandCheck_OnChange
        )
        self.checkBox_MagicInstantUseRightHand.clicked.connect(
            self.MagicInstantUseRightHandCheck_OnChange
        )
        ...
        # Page "Built-in"
        self.tableWidget_BuiltInMacros.itemSelectionChanged.connect(
            self.BuiltInMacros_OnSelect
        )
        ...
        self.comboBox_ControlKeyStrongAttack.activated.connect(
            self.ControlKeys_OnChange
        )
        ...
        self.spinBox_StandardPauseTime.valueChanged.connect(
            self.StandardPauseTime_OnChange
        )
    ...
    def hook_hotkeys(self):
        ...
        for vk, func in sorted(list_to_hotkeys, key=lambda x: len(x[0]), reverse=True):
    ...
    def refresh_currents(self):
        ...
        self.savefile.make_journal_entry(
            'Current positions for "Semi-manual" modes were refreshed.'
        )
        self.Pages_Refresh_Journal()
    ...
    def set_font_size(self) -> None:
        ...
        for name, obj in inspect.getmembers(self):
            if (
                ...
                obj.setFont(
                    PyQt5.QtGui.QFont(
                        font_family, font_size + self.savefile.font_size_adjustment
                    )
                )
    ...
    def SaveSlotsComboBox_Refresh(self):
        ...
        if saveslots:
            for saveslot in saveslots:
                self.comboBox_SaveSlots.addItem(f"{saveslot.number}. {saveslot.name}")
            self.comboBox_SaveSlots.setEnabled(True)
            self.comboBox_SaveSlots.setCurrentIndex(
                self.savefile.current_saveslot.number - 1
            )
        else:
    ...
    def SaveSlots_OnChange(self):
        ...
        new_saveslot = next(
            (x for x in self.savefile.saveslots if x.number == slot_number), None
        )
    ...
    def tableWidget_Macros_OnSelect(self):
        ...
        self.current_macro = next(
            (x for x in self.savefile.current_saveslot.macros if x.id == macro_id),
            Macro(self.savefile.current_saveslot),
        )
    ...
    def MacrosTable_Refresh(self):
        ...
        self.button_DownMacros.setEnabled(
            len(self.savefile.current_saveslot.macros) > 0
        )
        self.button_DeleteMacros.setEnabled(
            len(self.savefile.current_saveslot.macros) > 0
        )
    ...
    def GitHubIssues_Click(self) -> None:
        ...
        webbrowser.open_new(
            "https://github.com/a-burlakov/EldenRing-MelinasFingers/issues"
        )
    ...
    def UpMacro_Click(self):
        ...
        previous_macro = next(
            (x for x in self.savefile.current_saveslot.macros if x.id == previous_id),
            None,
        )
        # Changing ids.
    ...
    def DownMacro_Click(self):
        ...
        next_macro = next(
            (x for x in self.savefile.current_saveslot.macros if x.id == next_id), None
        )
        ...
        self.current_macro.id, next_macro.id = next_macro.id, self.current_macro.id
    ...
    def SearchMode_OnChange(self):
        ...
        self.savefile.current_saveslot.search_mode_equipment = (
            self.comboBox_EquipmentSearchMode.currentText().lower()
        )
        self.savefile.current_saveslot.search_mode_magic = (
            self.comboBox_MagicSearchMode.currentText().lower()
        )
        self.savefile.current_saveslot.search_mode_items = (
            self.comboBox_ItemsSearchMode.currentText().lower()
        )
    ...
    def Pages_Equipment_Table_Refresh(self) -> None:
        ...
        if not manual_mode or choosing_now:
            ...
            table_Equipment.setEditTriggers(
        ...
        if "weapon" in self.equipment_current_cell:
            ...
            manual_mode_for_current = (
                self.savefile.current_saveslot.armor_head_manual_mode
            )
        elif "chest" in self.equipment_current_cell:
            manual_mode_for_current = (
                self.savefile.current_saveslot.armor_chest_manual_mode
            )
        elif "arms" in self.equipment_current_cell:
            manual_mode_for_current = (
                self.savefile.current_saveslot.armor_arms_manual_mode
            )
        elif "legs" in self.equipment_current_cell:
            manual_mode_for_current = (
                self.savefile.current_saveslot.armor_legs_manual_mode
            )
        elif "talisman" in self.equipment_current_cell:
            manual_mode_for_current = (
                self.savefile.current_saveslot.talismans_manual_mode
            )

        if (choosing_now and manual_mode_for_current) or (
            not choosing_now and manual_mode
        ):
        ...
        for i, equip in enumerate(collection):
            ...
            table_Equipment.setItem(i // 5, i % 5, QTableWidgetItem(equip["name"]))
    ...
    def Pages_Equipment_Buttons_Refresh(self) -> None:
        ...
        if is_choosing_now:
            if "weapon" in current_cell:
                self.checkBox_Equipment_ManualMode.setChecked(
                    self.savefile.current_saveslot.weapons_manual_mode
                )
        ...
        self.button_EquipmentReloadInventory.setHidden(
            is_manual_mode or is_choosing_now
        )
            ...
            if is_choosing_now:
                if name_as_current == current_cell:
                    picture.setStyleSheet(
                        "QGraphicsView{border-image: url(:/newPrefix/images/"
                        + picture_name
                        + "_active.png);}"
                    )
                else:
                ...
                picture.setStyleSheet(
                    ...
                    + picture_name
                    + ".png);}"
                    "QGraphicsView:hover{border-image: url(:/newPrefix/images/"
                    + picture_name
                    + "_active.png);}"
                )
    ...
    def EquipmentInventoryType_OnChange(self) -> None:
        ...
        self.savefile.current_saveslot.current_equipment_type = (
            self.comboBox_EquipmentInventoryCurrentType.currentText()
        )
    ...
    def Equipment_ManualMode_Add(self) -> None:
        ...
        equipment_type_collections_accordance = {
            ...
            "Armor (chest)": (
                "chest",
                self.savefile.current_saveslot.armor_chest_manual,
            ),
    ...
    def Equipment_NotEnoughStats_OnChange(self) -> None:
        ...
        settings[self.equipment_current_cell][
            "not_enough_stats"
    ...
    def Equipment_ManualMode_Table_OnChange(self) -> None:
        ...
        for row in range(model.rowCount()):
            for i in range(5):
                ...
                if name == "None":
                    name = ""
                manual_list.append({"name": name, "position": position})
    ...
    def Pages_Refresh_Magic(self) -> None:
        ...
        self.checkBox_MagicInstantUseLeftHand.setChecked(
            magic_settings["instant_cast_left"]
        )
        self.checkBox_MagicInstantUseRightHand.setChecked(
            magic_settings["instant_cast_right"]
        )
    ...
    def MagicInstantUseRightHandCheck_OnChange(self) -> None:
        ...
        """
        self.current_macro.settings["magic"][
            "instant_cast_right"
    ...
    def AvailableSpells_RefillTable(self) -> None:
        ...
        for i, spell in enumerate(self.savefile.current_saveslot.spells):
            ...
            self.tableWidget_AvaiableMagic.setItem(i, 0, QTableWidgetItem(spell_name))
    ...
    def ItemInstantUse_OnChange(self) -> None:
        ...
        is changed.
        """
        self.current_macro.settings["items"][
            "instant_use"
    ...
    def Pages_Refresh_Builtin(self) -> None:
        ...
        current_builtin_macro_name = self.current_macro.settings["built-in"][
            "macro_name"
        ]
        ...
        if current_builtin_macro_name:
            builtin_macro = next(
                (
                    x
                    for x in built_in_macros()
                    if x["name"] == current_builtin_macro_name
                ),
                None,
            )
        ...
        if current_builtin_macro_name:
            for i, item in enumerate(built_in_macros()):
                if (
                    item["name"] == current_builtin_macro_name
    ...
    def Pages_Refresh_DIY(self) -> None:
        ...
        self.spinBox_DIYTimes.setValue(
            self.current_macro.settings["diy"]["times_to_repeat"]
        )
    ...
    def Pages_Refresh_Settings(self) -> None:
        ...
        self.comboBox_ControlKeyMove_Up.setCurrentText(
            self.savefile.game_controls["move_up"]
        )
        self.comboBox_ControlKeyMove_Down.setCurrentText(
            self.savefile.game_controls["move_down"]
        )
        self.comboBox_ControlKeyMove_Left.setCurrentText(
            self.savefile.game_controls["move_left"]
        )
        self.comboBox_ControlKeyMove_Right.setCurrentText(
            self.savefile.game_controls["move_right"]
        )
        ...
        self.comboBox_ControlKeyCrouch.setCurrentText(
            self.savefile.game_controls["crouch"]
        )
        self.comboBox_ControlKeyResetCamera.setCurrentText(
            self.savefile.game_controls["reset_camera"]
        )
        self.comboBox_ControlKeyAttack.setCurrentText(
            self.savefile.game_controls["attack"]
        )
        self.comboBox_ControlKeyStrongAttack.setCurrentText(
            self.savefile.game_controls["strong_attack"]
        )
        self.comboBox_ControlKeySkill.setCurrentText(
            self.savefile.game_controls["skill"]
        )
        self.comboBox_ControlKeySwitchItem.setCurrentText(
            self.savefile.game_controls["switch_item"]
        )
        self.comboBox_ControlKeySwitchSpell.setCurrentText(
            self.savefile.game_controls["switch_spell"]
        )
        self.comboBox_ControlKeyGuard.setCurrentText(
            self.savefile.game_controls["guard"]
        )
        self.comboBox_ControlKeyUseItem.setCurrentText(
            self.savefile.game_controls["use_item"]
        )
        self.comboBox_ControlKeyUse.setCurrentText(
            self.savefile.game_controls["event_action"]
        )
    ...
    def Pages_Refresh_Journal(self) -> None:
        ...
        # Scrolling to bottom.
        self.table_Journal.verticalScrollBar().setValue(
            self.table_Journal.verticalScrollBar().maximum()
        )
    ...
    def fill_builtin_macros(self) -> None:
        ...
        for i, builtin_macro in enumerate(built_in_macros()):
            ...
            self.tableWidget_BuiltInMacros.setItem(
                i, 0, QTableWidgetItem(builtin_macro["name"])
            )
        self.tableWidget_BuiltInMacros.blockSignals(False)
    ...
    def ControlKeys_OnChange(self):
        ...
        self.savefile.game_controls[
            "move_up"
        ] = self.comboBox_ControlKeyMove_Up.currentText().lower()
        self.savefile.game_controls[
            "move_down"
        ] = self.comboBox_ControlKeyMove_Down.currentText().lower()
        self.savefile.game_controls[
            "move_left"
        ] = self.comboBox_ControlKeyMove_Left.currentText().lower()
        self.savefile.game_controls[
            "move_right"
        ] = self.comboBox_ControlKeyMove_Right.currentText().lower()
        self.savefile.game_controls[
            "roll"
        ] = self.comboBox_ControlKeyRoll.currentText().lower()
        self.savefile.game_controls[
            "jump"
        ] = self.comboBox_ControlKeyJump.currentText().lower()
        self.savefile.game_controls[
            "crouch"
        ] = self.comboBox_ControlKeyCrouch.currentText().lower()
        self.savefile.game_controls[
            "reset_camera"
        ] = self.comboBox_ControlKeyResetCamera.currentText().lower()
        self.savefile.game_controls[
            "attack"
        ] = self.comboBox_ControlKeyAttack.currentText().lower()
        self.savefile.game_controls[
            "strong_attack"
        ] = self.comboBox_ControlKeyStrongAttack.currentText().lower()
        self.savefile.game_controls[
            "skill"
        ] = self.comboBox_ControlKeySkill.currentText().lower()
        self.savefile.game_controls[
            "switch_item"
        ] = self.comboBox_ControlKeySwitchItem.currentText().lower()
        self.savefile.game_controls[
            "switch_spell"
        ] = self.comboBox_ControlKeySwitchSpell.currentText().lower()
        self.savefile.game_controls[
            "guard"
        ] = self.comboBox_ControlKeyGuard.currentText().lower()
        self.savefile.game_controls[
            "use_item"
        ] = self.comboBox_ControlKeyUseItem.currentText().lower()
        self.savefile.game_controls[
            "event_action"
        ] = self.comboBox_ControlKeyUse.currentText().lower()
    ...
    def DIYTimes_OnChange(self) -> None:
        ...
        self.current_macro.settings["diy"][
            "times_to_repeat"
        ] = self.spinBox_DIYTimes.value()
    ...
    def set_macro_name_from_settings(self):
        ...
        if self.current_macro.type == "Magic":
            ...
            if self.current_macro.name != spell_name and (
                ...
                in [self.current_macro.standard_name(), "< empty >"]
                or any(
                    x["name"] == self.current_macro.name
                    for x in built_in_macros()
                    + self.savefile.current_saveslot.spells
                    + self.savefile.current_saveslot.items
                )
        ...
        if self.current_macro.type == "Built-in":
            ...
            built_in_macro_name = items[0].text()
            if self.current_macro.name != built_in_macro_name and (
                self.current_macro.name
                in [self.current_macro.standard_name(), "< empty >"]
                or any(
                    x["name"] == self.current_macro.name
