...
class Macro:
    ...
    def execute(self):
        ...
        self.savefile.make_journal_entry(
            f"Duration: {str(round(time_end - time_start, 5))}"
        )
        self.savefile.make_journal_entry("-" * 40)
    ...
    def form_keyline_magic(self):
        ...
        self.savefile.make_journal_entry(
            f"Current spell now - {self.saveslot.current_spell}"
        )
    ...
    def form_keyline_items(self):
        ...
        self.savefile.make_journal_entry(
            f"Current item now - {self.saveslot.current_item}"
        )
    ...
    def form_keyline_builtin(self):
        ...
        built_in_macro = next(
            x for x in built_in_macros() if x["name"] == built_in_macro_name
        )
    ...
    def form_keyline_diy(self):
        ...
        for command in commands_list:
            ...
            if keyline == "":
                macro = next(
                    (x for x in built_in_macros() if x["name"].lower() == command), None
                )
    ...
    def execute_keyline(self) -> None:
        ...
        for i, key_press in enumerate(key_presses):
            ...
            for key_press in keys_list:
                if key_press.strip() == "":
                    self.savefile.make_journal_entry(
                        "Had an empty key press, so it was skipped."
                    )
                    continue
...
def keyline_to_choose_next_weapon(weapons_pass: int = 0, left_hand=False) -> str:
    ...
    keyline = (
        f'esc|e|{"down|" if left_hand else ""}e|{"right|" * right_amount}pause20|e|esc'
    )
    return keyline
...
def keyline_to_choose_previous_weapon(weapons_pass: int = 0, left_hand=False) -> str:
    ...
    keyline = (
        f'esc|e|{"down|" if left_hand else ""}e|{"left|" * left_amount}pause20|e|esc'
    )
